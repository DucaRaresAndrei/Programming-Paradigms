...
rectangle width height = \point -> let (x, y) = point
...
-}
circle :: Float -> Region
circle radius = \point -> let (x, y) = point
                          in x^2 + y^2 <= radius^2
...
plot width height region =
    let rows = [ [ if region (fromIntegral x, fromIntegral y) then '*' else '.' | x <- [-width .. width] ]
...
{-
    *** TODO ***
    ...
    Răspuns: ...............
    Evaluarea lenesa are parte de implementari eficiente, bazate strict pe necesitate.
    In cazul nostru, avem parte de o reducere a costurilor de calcul atunci cand punctele
    sunt localizate devreme in regiunile unite.
-}
circles :: Int -> Region
circles n
    | n <= 0    = const False
...
{-
    *** TODO ***
    ...
    Răspuns: ...............
    Evaluarea va genera si verifica noile cercuri la infinit, fara o conditie de oprire.
-}
...
bfs start expand = bfs' [(start, 0)] S.empty
  where
    ...
    bfs' ((state, dist):queue) visited
      | state `S.member` visited = bfs' queue visited
      | otherwise = (state, dist) : bfs' (queue ++ nextStates) (S.insert state visited)
      where
        nextStates = [(s, dist + 1) | s <- expand state]
