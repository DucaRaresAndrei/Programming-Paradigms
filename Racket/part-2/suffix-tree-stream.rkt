...
; st-empty? : ST -> Bool
(define (st-empty? st)
  (define (is-empty-label list)
    (cond
      ((null? list) #t)
      ((list? list) (and (is-empty-label (car list)) (is-empty-label (cdr list))))
      (else #f)))

  (define (is-empty-subtree flux)
    (if (stream-empty? flux)
      #t
      (let is-empty-branches ((flux-branch flux))
        (let* ((branch (first-branch flux-branch))
              (label (get-branch-label branch))
              (subtree (get-branch-subtree branch)))
          (if (null? branch)
            (is-empty-branches (other-branches flux-branch))
            (if (and (is-empty-label label) (is-empty-subtree subtree))
              (is-empty-branches (other-branches flux-branch))
              #f))))))

  (or (stream-empty? st) (is-empty-subtree st)))
...
(define (get-ch-branch st ch)
  (let ((result (stream-first (stream-filter (lambda (branch) (and (not (null? branch)) (equal? ch (car (get-branch-label branch))))) st))))
    (if (null? result)
      #f
      result)))
