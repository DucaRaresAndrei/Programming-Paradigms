...
(define (collection-empty? col)
    ...
    (define (is-empty-collection collection)
        (if (stream-empty? collection)
            #t
            (if (is-empty (collection-car collection))
                (is-empty-collection (collection-cdr collection))
                #f)))

    (or (stream-empty? col) (is-empty-collection col)))


(define (collection-car col)
    (stream-first col))

(define (collection-cdr col)
    (stream-rest col))

(define (collection-map f col)
    (stream-map f col))

(define (collection-filter f col)
    (stream-filter f col))

(define (collection-list->stream L)
  (if (null? L)
      empty-stream
      (collection-cons (car L) (collection-list->stream (cdr L)))))
