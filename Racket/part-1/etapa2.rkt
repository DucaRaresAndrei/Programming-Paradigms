...
(define (get-ch-words words ch)
  (if (null? words)
    '()
    (filter (lambda (word) (and (equal? (car word) ch) (not (null? word)))) words)))
...
(define (ast-func suffixes)
  (if (null? suffixes)
    '()
    (cons (cons (car (car suffixes)) '()) (map (lambda (word) (drop word 1)) suffixes))))
...
(define (suffixes->st labeling-func suffixes alphabet)
  (define (is-empty list)
    (cond
      ((null? list) #t)
      ((list? list) (and (is-empty (car list)) (is-empty (cdr list))))
      (else #f)))
    
  (if (is-empty suffixes)
    '()
    (let ((real-alphabet (filter (lambda (character) (not (equal? '() (get-ch-words suffixes character)))) alphabet)))
      (let ((sufixes-per-ch (map (lambda (ch) (get-ch-words suffixes ch)) real-alphabet)))
        (map (lambda (sufixes) 
  ...
  (lambda (text)
    (lambda (function)
      (let* ((text-with-$ (append text '(#\$)))
        ...
        (suffixes->st function suffixes alphabet)))))
